...
import avatarImg from '../assets/avatar.png';  // Import the avatar image
    ...
    <header className='max-w-screen-2xl px-4 py-6'>
      <nav className='flex justify-between items-center'>
        ...
        <div className='flex items-center md:gap-16 gap-4'>
          <Link to="/">
            <span className='text-2xl font-semibold text-blue-500'>fidmind</span>
          </Link>
              ...
              <>
                <button onClick={() => setIsDropdownOpen(!isDropdownOpen)}>
                  <img
                    ...
                    alt="User Avatar"
                    className={`size-4 rounded-full ${currentUser ? 'ring-2 ring-blue-500' : ''}`}
                  />
                </button>
                {/* Show dropdown */}
                {isDropdownOpen && (
        ...
        <div className="absolute left-0 right-0 mt-2 bg-white shadow-lg rounded-md z-40">
          <ul className="py-2 max-h-60 overflow-y-auto">
            {filteredBooks.map((book) => (
              <li key={book._id}>
                <Link to={`/books/${book._id}`} className="block px-4 py-2 text-sm text-blue-500 hover:bg-gray-100">
                  {book.title}
                </Link>
              </li>
            ))}
